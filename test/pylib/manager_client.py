...
class ManagerClient():
    ...
    async def before_test(self, test_case_name: str, test_log: Path) -> None:
        # Pytest uses 'test' handler to for output logs during tests.
        # So we are adding a handler for it here to write all logs to test_log file. Since logger instance is singleton,
        # pytest will use this one instead of creating a new one
    ...
    async def after_test(self, test_case_name: str, success: bool) -> None:
        ...
        logging.getLogger('test').removeHandler(self.test_log_fh)
