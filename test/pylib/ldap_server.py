...
def start_ldap(port: int, instance_root: Path, byte_limit: int):
    ...
    def finalize():
        ...
        # not checking the output because it fails if tests are stopped with ctrl+C since toxiproxy-server already stopped
        subprocess.Popen(['toxiproxy-cli', 'd', proxy_name])
